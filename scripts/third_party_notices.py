...
PROJECT_NAME = "OrionBelt Runner"
ROOT_PACKAGE = "orionbelt-runner"
MODULE_NAME = "orionbelt_runner"
...
# Whether a container image is published. False turns off everything that
# describes one — the redistribution paragraph, the platform-layer section, the
# in-image column — because a project shipping only a wheel hands nobody a binary
# copy of its dependencies, and a notice that claims otherwise is simply untrue.
PUBLISHES_IMAGE = True

# Whether the rendered notice is committed to the repository.
#
# False keeps the licence gate and drops the artifact. That is the right trade
# wherever nothing is redistributed: the file is then almost entirely licence
# texts nobody is owed, churning on every dependency bump, while the half that
# earns its keep — refusing a dependency whose terms nobody reviewed — costs
# nothing to run. The file can still be produced on demand for a look; it is
# simply not part of the tree. Set this True the moment an artifact starts
# handing those packages over, because the notice stops being documentation at
# that point and becomes an obligation.
COMMIT_NOTICE = True
...
# own `ARG` default is not the whole answer. Empty here: this repo's Dockerfile names
# its extras literally, so there is nothing to resolve.
#
# A sibling that builds a matrix (`--build-arg OB_EXTRA=flight` alongside the default
# `flight-duckdb-only`) cannot be described by one value, because both images are
# published and the notice has to cover what either of them hands over. Declare the
# union there, not the default — the default is what a local `docker build` produces,
# not what the registry holds.
...
PLATFORM_LINKED_LIBS = (
    "glibc, libffi,\n  libssl, libsqlite3, liblzma, readline, and Pango, which WeasyPrint "
    "draws\n  through"
)
